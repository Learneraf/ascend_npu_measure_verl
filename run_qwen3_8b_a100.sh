#!/usr/bin/env bash
# GRPO | Qwen3-8B | veRL main_ppo | Ascend NPU migration entry
set -xeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# This tree has models/data under /mnt/data/FlagPerf on the Ascend host.
...
MODEL_PATH=${MODEL_PATH:-${PROJ}/models/Qwen3-8B}
DATA_DIR=${DATA_DIR:-${PROJ}/data/gsm8k_verl}

# Optional venv activation for copied environments. The current Ascend image
# already provides torch_npu/vllm_ascend/verl globally, so absence is fine.
...
actor_micro_bsz=${actor_micro_bsz:-2}
logprob_micro_bsz=${logprob_micro_bsz:-4}

RUN_ID=qwen3_8b_grpo_ascend_bs${train_batch_size}_n${rollout_n}_resp${max_response_length}_$(date +%Y%m%d_%H%M%S)
