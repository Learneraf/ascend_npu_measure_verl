#!/usr/bin/env bash
# GRPO | Qwen3-8B | Fully Async (4+4 GPU split) | veRL 0.8.0 | A100 80GB
# Rollout (4 GPU) 和 Training (4 GPU) 同时运行，时间重叠
set -xeuo pipefail
...
RUN_ID=qwen3_8b_async_$(date +%Y%m%d_%H%M%S)
...
ROLLOUT_GPUS=4       # vLLM 推理（独立 server pool）
...
kl_loss_coef=0.001
rollout_n=4
rollout_tp=1          # TP=1 → rollout_pool 内 4 个独立 vLLM 副本

# 总 rollout prompts = 20 sync步 × trigger(4) × mini_batch(64) = 5120
total_rollout_steps=5120
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.project_name=flagperf_qwen3_8b_async \
...
python3 fill_tables.py     --log     "${LOG_FILE}"     --tables  results_tables.md     --mode    async     --thresh  ${staleness_threshold}
echo "[fill] results_tables.md updated (async run)"
