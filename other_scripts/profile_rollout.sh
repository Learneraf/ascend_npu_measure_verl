#!/usr/bin/env bash
# Rollout internal profiling run: 5 steps, torch profiler on vLLM workers.
# Produces torch trace JSONs at PROF_DIR/agent_loop_rollout_replica_*/
# Usage: bash profile_rollout.sh [extra hydra overrides]
set -xeuo pipefail
...
RUN_ID=qwen3_8b_grpo_bs256_n4_resp1024_profile_$(date +%Y%m%d_%H%M%S)
...
python3 -m verl.trainer.main_ppo_sync \
    ...
    trainer.project_name=flagperf_rollout_profile \
...
echo "[profile_rollout] Done. Log: ${LOG_FILE}"
echo "[profile_rollout] Traces at: ${PROF_DIR}"
...
echo "[fill] results_tables.md updated (profile run)"
